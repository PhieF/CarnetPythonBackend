...
class RecentDBManager():
    ...
    def getRecentDBFile(self, id, mode, create=False):
        try:
            return open(self.settingsManager.getNotePath()+"/quickdoc/recentdb/"+id, mode, encoding='utf8')
        except FileNotFoundError:
            if(not create):
                raise FileNotFoundError()
            Path(self.settingsManager.getNotePath()+"/quickdoc/recentdb/").mkdir( parents=True, exist_ok=True )
            file = open(self.settingsManager.getNotePath()+"/quickdoc/recentdb/"+id, mode, encoding='utf8')
            file.write(string)
            file.close()
            return self.getRecentDBFile(id, mode)
